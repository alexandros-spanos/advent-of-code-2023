...
grid.FlowAll();
Console.WriteLine(grid.MaxEnergized);
...
class Grid
{
    ...
    public int MaxEnergized { get; private set; } = -1;
    ...
    public void FlowAll()
    {
        ...
        {
            FlowScenario(0, columnIndex, LightDirection.Down);
        }
        for (var rowIndex = 0; rowIndex < Heigth; rowIndex++)
        {
            FlowScenario(rowIndex, 0, LightDirection.Right);
        }
        for (var columnIndex = 0; columnIndex < Width; columnIndex++)
        {
            FlowScenario(Heigth - 1, columnIndex, LightDirection.Up);
        }
        for (var rowIndex = 0; rowIndex < Heigth; rowIndex++)
        {
            FlowScenario(rowIndex, Width - 1, LightDirection.Left);
        }
    }

    private void FlowScenario(int rowIndex, int columnIndex, LightDirection direction)
    {
        var initial = new Position(rowIndex, columnIndex);
        FlowBeam(initial, direction);
        var count = CountEnergized();
        if (MaxEnergized == -1 || MaxEnergized < count) MaxEnergized = count;
        ClearBeams();
    }
    ...
    public void ClearBeams()
    {
        ...
        _horizontalBeamsByRow.Clear();
        _verticalBeamsByColumn.Clear();
        foreach (var mirror in _mirrors)
        {
            mirror.OutwardBeams.Clear();
            mirror.InwardBeams.Clear();
        }
    }
